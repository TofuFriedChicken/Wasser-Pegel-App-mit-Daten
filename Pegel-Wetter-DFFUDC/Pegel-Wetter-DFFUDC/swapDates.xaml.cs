...
namespace Pegel_Wetter_DFFUDC
{
    public partial class swapDates : ContentPage
    {
        ...
        private void DateBack_Clicked(object sender, EventArgs e)
        {
            ...
            {
                DisplayAlert("Fehler", "Daten werden nur von den letzten 20 Tagen abgerufen.", "Schlieﬂen");
            }
        ...
        private async void DateForward_Clicked(object sender, EventArgs e)
        {
            ...
            {
                //Unittest
                await DisplayAlert("Fehler", "Du bist beim aktuellen Datum angekommen.", "Schlieﬂen");
            }
            else
            {
        ...
        //Quelle ChatGPT 4.0, 25.07.2024; mitlerweile mehrfach ver‰ndert
        ...
        {
            await ShowLoadingPopup(async () => //soll Ladefenster ˆffnen
            {
                await AddPinsRainfall(); //f¸gt geladene pins aus API in Liste hinzu

                //f¸gt pins auf map hinzu
        ...
        private async Task AddPinsRainfall()
        {
            ...
            //¸bergibt stationen an Methode, l‰d informationen in Liste aus stationen
        ...
        private void LoadRainPins(RainfallStations[] stations)
        {
            if (stations == null)
            {
                DisplayAlert("Fehler", "Die Liste ist leer. Daten kˆnnen nicht ausgegeben werden", "Ok");
            }
        ...
        {
            //zeigt Ladefenster solange (() => {Funktion}) l‰d     Lambda = (parameters) =>"goes to" {expression_or_statement_block} -> zum schreiben anonymer Funktionen (= Funktionen ohne Name, die gleich definiert werden, oft einmalig/kurzfristig)
            
            await ShowLoadingPopup(async () =>
            {   /* sollte aktuelles datum abfragen und aktualisieren*/

                //f¸gt geladene pins aus API in Liste hinzu
        ...
        private async Task LoadWaterPins()
        {
            try
            {
                ...
                */
                ...
                {
                    foreach (var position in WlModel._positions) //exeption bei Button wlMap, gibt null zur¸ck
                    {
